...
const MIGRATION_PATH = resolve(__dirname, '../../../db/migrations/0007_dashboard.sql');
...
async function main(): Promise<void> {
  ...
  console.log(`Dashboard migration complete. Applied ${applied}, skipped ${skipped}.`);
}

main()
  .catch((error) => {
    const message = error instanceof Error ? error.message : String(error);
    console.error(`Dashboard migration failed: ${message}`);
    process.exit(1);
  })
  .finally(async () => {
