  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      request<CommentResponse>(`/api/admin/comments?${params.toString()}`, {
        actionName: 'dashboard.comments.list',
  ...
  const handleBulk = async (action: string) => {
    try {
      ...
      if (action === 'Approve selected') {
        ...
        setComments((prev) =>
          prev.map((comment) =>
            selectedList.includes(comment.id) && !failedIds.has(comment.id)
              ? { ...comment, status: 'APPROVED' }
              : comment,
          ),
        );
        setSelectedIds(failedIds);
        setLoadError(bulkActionMessage(result, 'Approve'));
        ...
        setSelectedIds(failedIds);
        setLoadError(bulkActionMessage(result, 'Move to Trash'));
        if (result.failureCount === 0) {
          setIsBulkOpen(false);
          setIsSelectMode(false);
        }
        ...
        const failedIds = new Set(result.failures.map((entry) => entry.id));
        const succeededIds = new Set(selectedList.filter((id) => !failedIds.has(id)));
        setComments((prev) =>
          prev.filter(
            (comment) =>
              !succeededIds.has(comment.id) &&
              !(comment.parentId && succeededIds.has(comment.parentId)),
          ),
        );
        setSelectedIds(failedIds);
