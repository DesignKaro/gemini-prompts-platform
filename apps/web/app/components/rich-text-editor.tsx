...
import {
  ...
  MdFormatQuote, MdFormatStrikethrough, MdFormatUnderlined, MdInsertLink,
  MdRedo, MdUndo, MdClose, MdFormatAlignLeft, MdFormatAlignCenter,
  MdFormatAlignRight, MdFormatAlignJustify, MdFormatClear, MdCode,
  MdHorizontalRule, MdSubscript, MdSuperscript, MdImage, MdSmartToy,
} from 'react-icons/md';
...
const extensions = [
  ...
  Underline,
  Link.configure({ openOnClick: false, autolink: true, linkOnPaste: true, HTMLAttributes: { rel: null, target: null } }),
...
/* ─── Toolbar button ─── */
function TBtn({ active, onClick, disabled = false, title, children }: {
...
function FixedPopover({ anchor, onClose, children }: {
  anchor: Rect | null; onClose: () => void; children: React.ReactNode;
}) {
  useEffect(() => {
  ...
  return createPortal(
    <div data-popover style={style} className="rounded-[14px] border border-gray-100 bg-white shadow-2xl">
      {children}
    </div>,
    document.body,
  );
  ...
  return (
    <>
      <button
        ...
        type="button" title="Highlight colour" disabled={disabled}
        className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] text-[1.1rem] transition-colors
          ${isActive ? 'bg-[#e8f0ff] text-[#2e5edb]' : 'text-[#4b5565] hover:bg-[#eef2f8]'}
          ${disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer'}`}
        onMouseDown={(e) => { e.preventDefault(); if (!disabled) toggle(); }}
      >
        <BiHighlight />
      </button>
      ...
      {anchor && (
        <FixedPopover anchor={anchor} onClose={() => setAnchor(null)}>
          <div className="flex flex-col gap-2 p-3">
            <p className="text-[0.7rem] font-semibold uppercase tracking-wider text-gray-400">Highlight</p>
            <div className="flex gap-2">
              ...
              <button
                ...
                onMouseDown={(e) => { e.preventDefault(); editor?.chain().focus().unsetHighlight().run(); setAnchor(null); }}
...
function ImagePicker({ editor, disabled }: { editor: Editor | null; disabled: boolean }) {
  ...
  const close = () => { setIsOpen(false); setUrl(''); setAlt(''); setSelectedImage(null); setLibraryAltText(''); };

  const insert = (src: string, altText: string = alt) => {
    if (!src || !editor) return;
    editor.chain().focus().insertContent({ type: 'image', attrs: { src, alt: altText } }).run();
    close();
  };
  ...
  return (
    <>
      <button
        type="button" title="Insert Image" disabled={disabled}
        className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] text-[1.1rem] transition-colors
          ${isOpen ? 'bg-[#e8f0ff] text-[#2e5edb]' : 'text-[#4b5565] hover:bg-[#eef2f8]'}
          ${disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer'}`}
        onMouseDown={(e) => { e.preventDefault(); if (!disabled) setIsOpen(true); }}
      >
        <MdImage />
      </button>

      {isOpen && typeof window !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-[#0f1116]/40 backdrop-blur-sm p-4 animate-in fade-in duration-200" onMouseDown={close}>
          <div 
          ...
          >
            ...
            <div className="flex flex-col border-b border-[#e2e6ee] bg-gray-50/50 pt-4 px-6 relative shrink-0">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[1.3rem] font-semibold tracking-tight text-[#0f1116]">Insert Media</h3>
                <button 
                  type="button" 
                  onClick={close} 
              ...
              <div className="flex gap-6 relative bottom-[-1px]">
                {(['upload', 'library', 'url'] as const).map((t) => (
                  <button
                    ...
                    onClick={() => setTab(t)}
                  >
                    {t === 'upload' ? 'Upload Files' : t === 'library' ? 'Media Library' : 'Insert from URL'}
                  </button>
                ))}
              </div>
            </div>
              ...
              {tab === 'upload' && (
                <div className="flex-1 flex flex-col items-center justify-center p-8 bg-gray-50/30">
                  ...
                  >
                    ...
                    <div>
                      <h4 className="text-[1.3rem] font-medium text-[#0f1116]">Drop files to upload</h4>
                      <p className="text-[1rem] text-gray-500 mt-2">or <span className="text-[#1e4fd2] font-medium group-hover:underline">click here</span> to browse your computer</p>
                    </div>
                    <span className="text-[0.85rem] rounded-full bg-gray-100 px-4 py-1.5 text-gray-500 mt-4">Maximum file size: 10MB</span>
                  ...
                  <div className="flex-1 overflow-y-auto p-5 no-scrollbar bg-white">
                    {isLoadingLibrary ? (
                      ...
                      <div className="flex-1 h-full flex gap-3 flex-col items-center justify-center text-red-500 bg-red-50 rounded-[16px] border border-red-100 max-w-md mx-auto p-8 my-auto">
                        ...
                        <button type="button" onClick={loadLibrary} className="rounded-full bg-white border border-red-200 px-4 py-2 text-[0.85rem] text-red-600 hover:bg-red-50 font-medium shadow-sm">Retry Connection</button>
                      </div>
                      ...
                      <div className="flex-1 h-full flex flex-col items-center justify-center text-gray-500 gap-4">
                        ...
                        <div className="text-center max-w-xs">
                          <h4 className="text-[1.2rem] font-medium text-[#0f1116]">No items found</h4>
                          <p className="text-[0.95rem] mt-2 text-gray-400">Your media library is currently empty. Switch to the Upload tab to add some images.</p>
                        </div>
                        <button onClick={() => setTab('upload')} className="mt-4 rounded-[12px] bg-[#d5ea52] px-6 py-2.5 text-[0.95rem] font-medium text-[#0f1116] shadow-sm hover:opacity-90 transition-opacity">
                          Upload Files
                        </button>
                      </div>
                    ) : (
                  ...
                  {selectedImage && (
                    <div className="w-[340px] border-l border-[#e2e6ee] bg-gray-50/50 flex flex-col shrink-0 animate-in slide-in-from-right-8 duration-300">
                      <div className="p-5 border-b border-[#e2e6ee]">
                        <h4 className="font-semibold text-[#0f1116] text-[1.05rem]">Attachment Details</h4>
                      </div>
                      <div className="flex-1 overflow-y-auto p-5 space-y-5 no-scrollbar">
                        <div className="aspect-video w-full overflow-hidden rounded-[12px] border border-[#e2e6ee] bg-white shadow-sm">
                          <img src={selectedImage.url} alt={selectedImage.title} className="h-full w-full object-contain" />
                        </div>
                        <div>
                          <p className="text-[0.85rem] font-medium text-[#0f1116] break-all truncate" title={selectedImage.title}>{selectedImage.title}</p>
                          <div className="flex gap-4 mt-2">
                            <button 
                              className="text-[0.8rem] text-[#1e4fd2] hover:underline flex items-center gap-1 font-medium"
                              onClick={async () => {
                                await navigator.clipboard.writeText(selectedImage.url).catch(() => undefined);
                              }}
                            >
                              Copy URL
                            </button>
                            <a href={selectedImage.url} target="_blank" rel="noopener noreferrer" className="text-[0.8rem] text-gray-500 hover:text-[#0f1116] hover:underline flex items-center gap-1 font-medium">
                              View original file
                            </a>
                          </div>
                        </div>
                        ...
                        <div>
                          <label className="block text-[0.85rem] font-medium text-[#0f1116] mb-1.5">Alt Text</label>
                          <input
                            type="text"
                          ...
                          />
                          <p className="text-[0.75rem] text-gray-500 mt-2">Leave empty if decorative.</p>
                        </div>
                      </div>
                      ...
                      <div className="p-5 border-t border-[#e2e6ee] bg-white">
                        <button
                          type="button" 
                          onClick={() => insert(selectedImage.url, libraryAltText || selectedImage.title)}
              ...
              {tab === 'url' && (
                <div className="flex-1 flex flex-col items-center justify-center p-8 bg-gray-50/30">
                  <div className="w-full max-w-lg bg-white p-8 rounded-[20px] shadow-sm border border-[#e2e6ee]">
                    <div className="mb-6">
                      <h4 className="text-[1.3rem] font-semibold text-[#0f1116]">Insert from URL</h4>
                      <p className="text-[0.95rem] text-gray-500 mt-1">Paste a direct link to an image on the web.</p>
                    </div>
                    <div className="flex flex-col gap-5">
                      <div>
                        <label className="block text-[0.85rem] font-medium text-gray-700 mb-1.5">Image URL</label>
                        <input
                          ...
                          onKeyDown={(e) => { if (e.key === 'Enter') insert(url); }}
                        />
                      </div>
                      <div>
                        <label className="block text-[0.85rem] font-medium text-gray-700 mb-1.5">Alt Text (Optional)</label>
                        <input
                      ...
                      <div className="pt-3">
                        <button
                          type="button" onClick={() => insert(url)}
                          disabled={!url.trim()}
...
function LinkModal({ editor, disabled }: { editor: Editor | null; disabled: boolean }) {
  ...
  useEffect(() => { if (anchor) setTimeout(() => urlRef.current?.focus(), 10); }, [anchor]);
  ...
  const open = () => {
    ...
    } else { setLinkUrl(''); setNofollow(false); }
    const r = btnRef.current?.getBoundingClientRect();
    if (r) setAnchor({ top: r.top, left: r.left, bottom: r.bottom });
  };

  const close = () => { setAnchor(null); setLinkUrl(''); setNofollow(false); };

  const apply = () => {
    if (!editor) return;
    const url = linkUrl.trim();
    if (!url) { editor.chain().focus().extendMarkRange('link').unsetLink().run(); }
    else { editor.chain().focus().extendMarkRange('link').setLink({ href: url, rel: nofollow ? 'nofollow' : null, target: null }).run(); }
    close();
  };
  ...
  return (
    <>
      <button
        ref={btnRef}
        data-popover-trigger
        type="button" title="Insert / Edit Link" disabled={disabled}
        className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] text-[1.1rem] transition-colors
          ${(isLinkActive || anchor) ? 'bg-[#e8f0ff] text-[#2e5edb]' : 'text-[#4b5565] hover:bg-[#eef2f8]'}
          ${disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer'}`}
        onMouseDown={(e) => { e.preventDefault(); if (!disabled) open(); }}
      >
        <MdInsertLink />
      </button>
        ...
        <FixedPopover anchor={anchor} onClose={close}>
          <div className="w-[300px] p-5">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-[0.95rem] font-semibold text-gray-800">{isLinkActive ? 'Edit Link' : 'Add Link'}</h3>
              <button type="button" onClick={close} className="text-gray-400 hover:text-gray-700"><MdClose size={18} /></button>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') apply(); if (e.key === 'Escape') close(); }}
              ...
              <label className="flex cursor-pointer items-center gap-2">
                ...
                <span className="text-[0.85rem] text-gray-600">Nofollow <span className="text-gray-400">(SEO)</span></span>
              </label>
              <div className="flex gap-2">
                ...
                {isLinkActive && (
                  <button type="button"
                    onMouseDown={(e) => { e.preventDefault(); editor?.chain().focus().extendMarkRange('link').unsetLink().run(); close(); }}
...
const Toolbar: FC<{ editor: Editor | null }> = ({ editor }) => {
  ...
  const sync = useCallback((e: Editor | null) => {
    if (!e) {
      ...
      setIsSub(false); setIsSuper(false); setIsCode(false); setIsCodeBlock(false);
      setIsBlockquote(false); setIsBullet(false); setIsOrdered(false);
      setBlockType('paragraph'); setAlign('left'); setCanUndo(false); setCanRedo(false);
      return;
    }
    setIsBold(e.isActive('bold')); setIsItalic(e.isActive('italic'));
    setIsUnderline(e.isActive('underline')); setIsStrike(e.isActive('strike'));
    setIsSub(e.isActive('subscript')); setIsSuper(e.isActive('superscript'));
    setIsCode(e.isActive('code')); setIsCodeBlock(e.isActive('codeBlock'));
    setIsBlockquote(e.isActive('blockquote'));
    setIsBullet(e.isActive('bulletList')); setIsOrdered(e.isActive('orderedList'));
    const a = (['left','center','right','justify'] as AlignType[]).find(x => e.isActive({ textAlign: x }));
    setAlign(a ?? 'left');
    ...
    try { setCanUndo(e.can().undo()); } catch { setCanUndo(false); }
    try { setCanRedo(e.can().redo()); } catch { setCanRedo(false); }
  }, []);

  useEffect(() => {
    if (!editor) { sync(null); return; }
    const h = () => sync(editor);
    h();
  ...
  const setHeading = (val: TextBlockType) => {
    ...
    editor.chain().focus().setHeading({ level: Number(val[1]) as 1|2|3|4|5 }).run();
  };
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="no-scrollbar flex items-center gap-1 overflow-x-auto border-b border-[#e0e5ef] bg-[#f7f9fc] px-3 py-2">
        <TBtn title="Undo" disabled={d || !canUndo} onClick={() => editor?.chain().focus().undo().run()}><MdUndo /></TBtn>
        <TBtn title="Redo" disabled={d || !canRedo} onClick={() => editor?.chain().focus().redo().run()}><MdRedo /></TBtn>
        <Divider />
        <select value={blockType} disabled={d} aria-label="Text type"
        ...
        <Divider />
        <TBtn title="Align Left" active={align === 'left'} disabled={d} onClick={() => editor?.chain().focus().setTextAlign('left').run()}><MdFormatAlignLeft /></TBtn>
        <TBtn title="Align Centre" active={align === 'center'} disabled={d} onClick={() => editor?.chain().focus().setTextAlign('center').run()}><MdFormatAlignCenter /></TBtn>
        <TBtn title="Align Right" active={align === 'right'} disabled={d} onClick={() => editor?.chain().focus().setTextAlign('right').run()}><MdFormatAlignRight /></TBtn>
        <TBtn title="Justify" active={align === 'justify'} disabled={d} onClick={() => editor?.chain().focus().setTextAlign('justify').run()}><MdFormatAlignJustify /></TBtn>
      </div>

      {/* Row 2 */}
      <div className="no-scrollbar flex items-center gap-1 overflow-x-auto border-b border-[#e0e5ef] bg-[#f7f9fc] px-3 py-2">
        <TBtn title="Bold" active={isBold} disabled={d} onClick={() => editor?.chain().focus().toggleBold().run()}><MdFormatBold /></TBtn>
        <TBtn title="Italic" active={isItalic} disabled={d} onClick={() => editor?.chain().focus().toggleItalic().run()}><MdFormatItalic /></TBtn>
        <TBtn title="Underline" active={isUnderline} disabled={d} onClick={() => editor?.chain().focus().toggleUnderline().run()}><MdFormatUnderlined /></TBtn>
        <TBtn title="Strikethrough" active={isStrike} disabled={d} onClick={() => editor?.chain().focus().toggleStrike().run()}><MdFormatStrikethrough /></TBtn>
        <TBtn title="Subscript" active={isSub} disabled={d} onClick={() => editor?.chain().focus().toggleSubscript().run()}><MdSubscript /></TBtn>
        <TBtn title="Superscript" active={isSuper} disabled={d} onClick={() => editor?.chain().focus().toggleSuperscript().run()}><MdSuperscript /></TBtn>
        <Divider />
        <HighlightPicker editor={editor} disabled={d} />
        <Divider />
        <TBtn title="Bullet List" active={isBullet} disabled={d} onClick={() => editor?.chain().focus().toggleBulletList().run()}><MdFormatListBulleted /></TBtn>
        <TBtn title="Numbered List" active={isOrdered} disabled={d} onClick={() => editor?.chain().focus().toggleOrderedList().run()}><MdFormatListNumbered /></TBtn>
        <Divider />
        <TBtn title="Blockquote" active={isBlockquote} disabled={d} onClick={() => editor?.chain().focus().toggleBlockquote().run()}><MdFormatQuote /></TBtn>
        <TBtn title="Inline Code" active={isCode} disabled={d} onClick={() => editor?.chain().focus().toggleCode().run()}><MdCode /></TBtn>
        <TBtn title="Code Block" active={isCodeBlock} disabled={d} onClick={() => editor?.chain().focus().toggleCodeBlock().run()}>
          <span className="font-mono text-[0.72rem] font-bold">{'{}'}</span>
        </TBtn>
        <TBtn title="Horizontal Rule" disabled={d} onClick={() => editor?.chain().focus().setHorizontalRule().run()}><MdHorizontalRule /></TBtn>
        <Divider />
        <ImagePicker editor={editor} disabled={d} />
        <LinkModal editor={editor} disabled={d} />
        <Divider />
        <TBtn title="Insert Prompt Box" active={editor?.isActive('promptBox')} disabled={d} onClick={() => editor?.chain().focus().insertContent({ type: 'promptBox', content: [{ type: 'paragraph' }] }).run()}><MdSmartToy /></TBtn>
        <TBtn title="Clear Formatting" disabled={d} onClick={() => editor?.chain().focus().unsetAllMarks().clearNodes().run()}><MdFormatClear /></TBtn>
      </div>
    </div>
  );
};
...
function WordCountBar({ editor }: { editor: Editor | null }) {
  ...
  return (
    <div className="flex items-center gap-4 border-t border-[#e0e5ef] bg-[#f7f9fc] px-4 py-1.5 text-[0.75rem] text-[#9aa1ae] rounded-b-[19px]">
      <span><span className="font-semibold text-[#4b5565]">{words}</span> words</span>
      <span><span className="font-semibold text-[#4b5565]">{chars}</span> characters</span>
    </div>
  );
}
